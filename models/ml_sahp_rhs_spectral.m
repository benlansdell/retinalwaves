function rhs=ml_sahp_rhs_spectral(t,EVRt,dummy,allpars)
	...
	rhs=[reshape(fft2((1000/Cm)*(-gCa.*(V-VCa)-gK.*(V-VK)-gLM*(V-VL)-gACh.*(V-Vsyn)-noise(:,:,round(t/noisedt)+1).*(V-Vnoise))),nx*ny,1);...
		reshape(fft2((1/tauR)*(Lambda.*(Rinf-R)+alpha*S.*(1-R))),nx*ny,1);...
		reshape(fft2(-S/tauS   + c*G), nx*ny,1);...
